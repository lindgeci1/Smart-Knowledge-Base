...
export function useChatState() {
  ...
  const fetchConversations = useCallback(async () => {
    try {
      const response = await apiClient.get(`${API_BASE_URL}/Chat/GetAllChats`);
      
      // Transform backend format to frontend format
      const transformedConversations: Conversation[] = response.data.map((chat: any) => ({
        id: chat.chatId,
        title: chat.title,
  ...
  const fetchMessages = useCallback(async (chatId: string) => {
    try {
      const response = await apiClient.get(`${API_BASE_URL}/Chat/GetAllMessages/${chatId}`);
  ...
  const fetchDocuments = useCallback(async () => {
    try {
      ...
      const [documentsResponse, textsResponse] = await Promise.all([
        apiClient.get(`${API_BASE_URL}/Documents/summaries`),
        apiClient.get(`${API_BASE_URL}/Texts/summaries`)
      ]);
  ...
  const createNewChat = useCallback(async () => {
    try {
      const response = await apiClient.post(`${API_BASE_URL}/Chat/CreateChat`, {
        title: 'New Chat'
      });
  ...
  const selectConversation = useCallback(
    async (id: string) => {
      ...
      if (conversation && conversation.documentId) {
        ...
        const lockedDoc = availableDocuments.find((d) => d.id === conversation.documentId);
        if (lockedDoc) {
  ...
  const deleteConversation = useCallback(
    async (id: string) => {
      try {
        await apiClient.delete(`${API_BASE_URL}/Chat/DeleteChat/${id}`);
    ...
    async (content: string) => {
      ...
      try {
        const response = await apiClient.post(
          `${API_BASE_URL}/Chat/CreateMessage/${activeConversationId}`,
          {
            message: content,
        ...
        setMessages((prev) => {
          ...
          return {
            ...prev,
            [activeConversationId]: [...withoutTemp, userMsg, transformedAiMessage]
          };
        });
        ...
        if (currentChat && (
            ...
            (typeof currentChat.title === 'string' && currentChat.title.toLowerCase().startsWith('new'))
          )) {
          setConversations((prev) =>
            prev.map((c) =>
              c.id === activeConversationId
  ...
  useEffect(() => {
    ...
    if (conversation?.documentId) {
      ...
      if (selectedDocument?.id === conversation.documentId && isDocumentLocked) return;
